...
vim.keymap.set("n", "<space>sr", ":IronRepl<CR>", { desc = "Open REPL" })
...
-- Print keybinds on startup
local function keymap_help()
    print([[
Keybindings for Iron.nvim:
  <space>sr  → Open REPL
  <space>sl  → Send line
  <space>sc  → Send motion/selection
  <space>sf  → Send file
  <space>s<CR> → Send until cursor
  <space>si  → Interrupt REPL
  <space>sq  → Quit REPL
  <space>cl  → Clear REPL
]])
end
